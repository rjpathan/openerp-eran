...
class stock_move(osv.osv):
     ...
     def action_done(self, cr, uid, ids, context=None):
        ...
        if context is None:
            context = {}
        if ids and context.has_key('stock_move'):  
...
class stock_change_product_qty(osv.osv_memory):
   ...
   def default_get(self, cr, uid, fields, context):
	   ...
	   def_loc=self.pool.get('ir.values').search(cr,uid,[('name','=','location_id'),('key','=','default'),('model','=','stock.change.product.qty'),('user_id','=',uid)])
           def_loc_all=self.pool.get('ir.values').search(cr,uid,[('name','=','location_id'),('key','=','default'),('model','=','stock.change.product.qty'),('user_id','=',False)])
           if def_loc and def_loc_all:
               def_loc=max(def_loc[0],def_loc_all[0])
               location_id=int(self.pool.get('ir.values').browse(cr,uid,def_loc).value[1:-1].strip())
           elif def_loc or def_loc_all:
               def_loc=def_loc_all and def_loc_all[0] or def_loc and def_loc[0]
               location_id=int(self.pool.get('ir.values').browse(cr,uid,def_loc).value[1:-1].strip())        
	   else:
