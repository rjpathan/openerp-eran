...
class sale_order(osv.osv):
    ...
    def search(self, cr, uid, args, offset=0, limit=None, order=None,context=None, count=False):
        ...
        if args and args[0]=='draft':
            ...
            if user_obj.pos_config and user_obj.pos_config.shop_id and uid!=1:
...
#    def check_sale_import(self, cr, uid,context=None):
#         conf = self.pool.get('magento.configuration').search(cr,uid,[],context)
#         if conf:
#                 obj = self.pool.get('magento.configuration').browse(cr,uid,conf[0])
#                 no_of_orders = obj.no_of_sale_order
#                 if obj.last_imported_invoice_timestamp:
#                         a=str(obj.last_imported_invoice_timestamp)
#                         a=a.split('.')[0]
#                         pointofsale =obj.pointofsale
#                         b = datetime.now()
#                         a = datetime.strptime( a, "%Y-%m-%d %H:%M:%S" )
#                         c = b - a
#                         if c.seconds < 3600 and pointofsale:
#                              self.pool.get('magento.configuration').write(cr,uid,obj.id,{'pointofsale':False})
#                              return [True,no_of_orders]
#                         else: 
#                              return False
#                 return False               
...
class product_product(osv.osv):
    ...
    def _product_available(self, cr, uid, ids, field_names=None, arg=False, context=None):
        """ Finds the incoming and outgoing quantity of product.
         Inherited to add Pless customization for showing the different shops qty in point of sale    
        ...
        for f in field_names:
           ...
           #====================================================================
           # #Dantunes customization for showing the different shops qty in point of sale    
           #====================================================================
            if 'shop_id' in context.keys():
                    config_id = context['shop_id']['config_id'][0]
                    loca=self.pool.get('pos.config').browse(cr,uid,config_id).shop_id.warehouse_id.lot_stock_id.id
                    c={'lang': 'en_US', 'search_default_filter_to_sell': 1,'states': ('confirmed', 'waiting', 'assigned', 'done'), 'tz': 'Europe/Brussels', 'uid': uid, 'active_model': 'product.product', 'what': ('in', 'out'),  'bin_size': True, 'location': loca, 'active_ids': ids}
        ...
        'to_weight' : fields.boolean('To Weight', help="Check if the product should be weighted (mainly used with self check-out interface)."),
         ...
         'qty_available': fields.function(_product_available, multi='qty_available',
            ...
            help="Current quantity of products.\n"
                 ...
                 "with 'internal' type."),
            'other_store_qty':fields.function(_product_available,multi='other_store_qty', type='float', string='Quantity On Shop'),   
#            'other_store_qty2':fields.function(_product_available,multi='other_store_qty', type='float', string='Quantity On Shop')    
    }
    ...
    def search(self, cr, uid, args, offset=0, limit=None, order=None,context=None, count=False):
        ...
        if args:
            for first in args:
                if first[0]=='available_in_pos':
                    res =  super(product_product, self).search(cr, uid, args=args, offset=offset, limit=limit, order=order,context=context, count=count)
                    for id in res:
                        product_obj = self.pool.get('product.product').browse(cr,uid,id,context)
                        if product_obj.qty_available>0  or product_obj.type=='service':
                                resultpos.append(id)
                    return resultpos       
        res =  super(product_product, self).search(cr, uid, args=args, offset=offset, limit=limit, order=order,context=context, count=count)
        return res
