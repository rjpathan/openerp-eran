...
//	return round_pr(value, Math.pow(10,-decimals));
    }
    window.round = round;
    ...
    module.PosModel = Backbone.Model.extend({
                ...
                .then(function(users){
                    ...
                    return self.fetch(
                        ...
                        ['name', 'code', 'list_price','price','pos_categ_id', 'taxes_id', 'ean13', 
                         'to_weight', 'uom_id', 'uos_id', 'uos_coeff', 'mes_type', 'description_sale', 'description','sale_journal'],
                        [['sale_ok','=',true],['available_in_pos','=',true]],
                        {pricelist: self.get('shop').pricelist_id[0],shop_id:self.get('pos_session')} // context for price
                    );
        ...
        //Added by kiran.P for Gift Card
        add_new_gift_order: function(){
            var order = new module.Order({pos:this});
            this.get('orders').add(order);
            return order
        },
    ...
    module.Orderline = Backbone.Model.extend({
        ...
        set_quantity: function(quantity){
            if(quantity === 'remove' || quantity === ''){
            	...
            	if (this.product.get('sale_journal')){
            		alert('Nur 1 Gutschein kann in einem Auftrag erteilt werden!')
            	}
            	else{
...
//      Added for segrigation of taxes in pos recipt dantunes
        get_7tax: function(){
            return this.get_all_prices().tax7;
        },
        get_19tax: function(){
            return this.get_all_prices().tax19;
        },
//      End for segrigation of taxes in pos recipt dantunes       
...
//              End for segrigation of taxes in pos recipt dantunes 
            ...
            return {
                ...
                "tax": taxtotal,
                "tax7":tax7,             //        Added for segrigation of taxes in pos recipt dantunes
                "tax19":tax19,         //        Added for segrigation of taxes in pos recipt dantunes
            };
        },
    });
    ...
    module.Paymentline = Backbone.Model.extend({
        initialize: function(attributes, options) {
          ...
          //Added by kiran.P for credit memeo and gift card
            if (options.pos_ref){
            	this.pos_ref=options.pos_ref;
            }
            else{
            	this.pos_ref= '';
            }
            if (options.ref_num){
            	this.ref_num=options.ref_num;
            }
            else{
            	this.ref_num= '';
            }
        ...
        export_as_JSON: function(){
            return {
                ...
                //Added by kiran.P for credit memo and Gift Card
                pos_reference:this.pos_ref,
                type:this.type,
                //END
            };
        },
    ...
    module.Order = Backbone.Model.extend({
        ...
        //Added by kiran.P for Credit Memo
        addCreditNoteLine: function(args) {
            var paymentLines = this.get('paymentLines');
            var credit_journal= null;
            this.pos.get('cashRegisters').each(function(cashRegister) {
                if (cashRegister.get('journal').code.toLowerCase()==='cmemo')
                	credit_journal=cashRegister
                });
            var newPaymentline = new module.Paymentline({},{cashRegister:credit_journal,pos_ref:args[1],ref_num:args[2],type:'credit_note'});
            newPaymentline.set_amount( args[0] );
            paymentLines.add(newPaymentline);
        },
        
      //Added by kiran.P for Gift Card
        addGiftCardLine: function(args) {
            var paymentLines = this.get('paymentLines');
            var gift_journal= null;
            this.pos.get('cashRegisters').each(function(cashRegister) {
                if (cashRegister.get('journal').code.toLowerCase()==='gcpay')
                	gift_journal=cashRegister
                });
            var newPaymentline = new module.Paymentline({},{cashRegister:gift_journal,pos_ref:args[1],ref_num:args[2],type:'gift_card'});
            newPaymentline.set_amount( args[0] );
            paymentLines.add(newPaymentline);
        },
...
//        Added for segrigation of taxes in pos recipt dantunes
        get7Tax: function() {
            return (this.get('orderLines')).reduce((function(sum, orderLine) {
                return sum + orderLine.get_7tax();
            }), 0);
        },
        get19Tax: function() {
            return (this.get('orderLines')).reduce((function(sum, orderLine) {
                return sum + orderLine.get_19tax();
            }), 0);
//         End for segrigation of taxes in pos recipt dantunes
        },
