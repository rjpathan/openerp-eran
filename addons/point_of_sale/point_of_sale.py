...
#     ]
    ...
    def create(self, cr, uid, values, context=None):
        ...
        if not config_id:
            raise osv.except_osv( _('Error!'),
                _("Du musst ein Kassensystem für deine Sitzung wählen."))
        ...
        if not pos_config.journal_id:
            ...
            if jid:
                ...
                raise osv.except_osv( _('error!'),
                    _("Es ist nicht möglich die Sitzung zu öffnen. Du musst ein Verkaufjournal wählen."))
    ...
    def wkf_action_closing_control(self, cr, uid, ids, context=None):
        for session in self.browse(cr, uid, ids, context=context):
             ...
             #    raise osv.except_osv(_('Error!'), 
              #         _("The Difference amount should be Zero"))
    ...
    def wkf_action_close(self, cr, uid, ids, context=None):
        ...
        for record in self.browse(cr, uid, ids, context=context):
            if record.cash_register_difference!=0:
                 raise osv.except_osv(_('Error!'), 
                       _("Der Differenz Betrag muss Null sein."))
            for st in record.statement_ids:
...
#            Added by Kiran.p for Gift Card Journal
            for order in session.order_ids:
                if order.state=='paid':
                    if order.sale_journal.id!=session.config_id.journal_id.id:
                        gift_journal=order.sale_journal.id
                        gift_orders.append(order.id)
                    else:
                        normal_orders.append(order.id)
            if normal_orders:
                self.pool.get('pos.order')._create_account_move_line(cr, uid,normal_orders,session, move_id, context=context)
            if gift_orders:
                gift_move_id = self.pool.get('account.move').create(cr, uid, {'ref' : session.name, 'journal_id' : gift_journal, }, context=context)
                self.pool.get('pos.order')._create_account_move_line(cr, uid,gift_orders,session, gift_move_id, context=context)

            for order in session.order_ids:
                if order.state not in ('paid', 'invoiced'):
                    raise osv.except_osv(
                        _('Error!'),
                        _("Du kannst nicht alle Bestellungen der Sitzung bestätigen, weil sie nicht den Bezahl-Status haben."))
                else:
...
class sale_order(osv.osv):
    _inherit = 'sale.order'    
    _columns = {
                'has_attachment':fields.boolean("Attachments")
                }    
    def search(self, cr, uid, args, offset=0, limit=None, order=None,context=None, count=False):
        ...
        if args and args[0]=='draft':
            test=True
            shop = args[1]['name']
            shop_id = self.pool.get('sale.shop').search(cr,uid,[('name','=',shop)])
            args=[('magento_increment_id','!=',False),('state','=','draft')]
            user_obj = self.pool.get('res.users').browse(cr,uid,uid)
            if user_obj.pos_config and user_obj.pos_config.shop_id:
                   shop = user_obj.pos_config.shop_id.id
                   if uid==1:
                        shop = shop_id and shop_id[0]
                   args.append(('shop_id','=',shop))
        else:
            user_obj = self.pool.get('res.users').browse(cr,uid,uid)
            if uid!=1 and user_obj.pos_config and user_obj.pos_config.shop_id and args:
                   args.append(('shop_id','=',user_obj.pos_config.shop_id.id))           
            
        res =  super(sale_order, self).search(cr, uid, args=args, offset=offset, limit=limit, order=order,context=context, count=count)
        if test:
            count1=0
            count=0
            for obj in self.pool.get('sale.order').browse(cr,uid,res):
...
class pos_order(osv.osv):
    ...
    def button_dummy(self, cr, uid, ids, context=None):
        return True
    def create_from_ui(self, cr, uid, orders,money, context=None):
        #_logger.info("orders: %r", orders)
        order_ids = []
        values={}
      #  print "orders............",orders
       #========================================================
       #      For put and get money from pos u
       #========================================================
        test=True
        if money:
               money=money[0]
               session = self.pool.get('pos.session').browse(cr, uid, money[3], context=context)        
               account_id=False
               if test and money[0]:
                    for line_id in session.statement_ids:
                        if line_id.journal_id.type=='cash':
                            journal_id=line_id.journal_id.id
                            statement_id=line_id.id
                    for journal in session.config_id.journal_ids:
                            if  journal.type=='cash':
                                account_id=  journal.profit_account_id.id
                    if money[0] and money[1] and money[2] and money[3]:
                          values={'name':money[2],'journal_id':journal_id,'account_id':account_id,'ref':session.name,'statement_id':statement_id}
                          try:	
                              if money[0]=='out':
                                  values['amount'] = -float(money[1])
                              else:
                                  values['amount']=float(money[1])
                          except:
                              raise osv.except_osv(_("Warning!"),_("Bitte gebe einen gültigen Betrag ein."))
                          self.pool.get('account.bank.statement.line').create(cr, uid, values, context=context)    
                          test=False      
        #===================================================================
        #         ENding     
        #===================================================================
...
#            print order,'Order----------'
            ...
            pricelist =session.config_id.shop_id.pricelist_id.id
            order_id = self.create(cr, uid, {
                'name': order['name'],
                'user_id': order['user_id'] or False,
                'session_id': order['pos_session_id'],
                'lines': order['lines'],
                'sale_journal':session.config_id.journal_id.id,
                'pos_reference':order['name'],
                'partner_id':partner_id,
                'pricelist_id':pricelist,
            }, context)
#            Code added by Kiran.P for making order as Gift Card Journal
            prod_pool=self.pool.get('product.product')
            for line in order['lines']:
                prod_id=line[2]['product_id']
                prod_obj=prod_pool.browse(cr,uid,prod_id)
                if prod_obj.sale_journal:
                    if prod_obj.sale_journal.code.lower()=='gcard':
                        self.write(cr,uid,[order_id],{'sale_journal':prod_obj.sale_journal.id,'closed':False})
                        break
            memo_ref=[]
            amount=0.0
            for payments in order['statement_ids']:
                ...
                if payment['type']=='credit_note':
                    self.pool.get('account.move').write(cr,uid,[payment['pos_reference']],{'closed':True,'closed_date':time.strftime('%Y-%m-%d')})
                if payment['type']=='gift_card':
                    self.pool.get('pos.order').write(cr,uid,[payment['pos_reference']],{'closed':True,'redeem_date':time.strftime('%Y-%m-%d')})
                    refund_res=self.pool.get('pos.order').refund(cr,uid,[payment['pos_reference']],context)
                    refund_id=refund_res['res_id']
                    self.pool.get('pos.order').pay_gift_card(cr,uid,[refund_id],context)
...
#        'sale_journal': fields.related('session_id', 'config_id', 'journal_id', relation='account.journal', type='many2one', string='Sale Journal', store=True, readonly=True),
        'sale_journal': fields.many2one('account.journal','Sale Journal',readonly=True),
    }
        ...
        for order in self.browse(cr, uid, ids, context=context):
            ...
            for key, tax_amount in group_tax.items():
                ...
                insert_data('tax', {
                    ...
                    'tax_amount': tax_amount,
                    'partner_id': order.partner_id and self.pool.get("res.partner")._find_accounting_partner(order.partner_id).id or False
                }, True if tax_amount<0 else False)
